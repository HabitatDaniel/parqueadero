...
<body>
    ...
    if (isset($_GET['placa']) && !empty(trim($_GET['placa']))) {
        ...
        $sql = "SELECT Oid,  IdConductor,Marca,Codigo_Interno FROM patineta WHERE IdConductor = ?";
        ...
        if ($vehiculo = $resultado->fetch_assoc()) {
            ...
            echo "<p><strong>Modelo:</strong> " . htmlspecialchars($vehiculo['IdConductor']) . "</p>";
            echo "<p><strong>Placa:</strong> " . htmlspecialchars($vehiculo['Codigo_Interno']) . "</p>";

            // Definir la ruta de la carpeta de imágenes
            $carpeta_fotos = "../../../Soportes/Patineta/" .$vehiculo['Codigo_Interno'] . "/";
            $nombre_foto = $vehiculo['Codigo_Interno'] . ".JPG";
